...
def run(args):
    ...
    for record in records:
        ...
        # ADDED
        nurse = row['nurse']
        nurse = int(nurse) if is_integer(nurse) else np.nan

        site = row['site']
        site = int(site) if is_integer(site) else np.nan

        device_mapping = {device: (idx if pd.notna(device) else -1) for idx, device in enumerate(df['device'].unique())}
        device = device_mapping[row['device']] if pd.notna(row['device']) and device_mapping[row['device']] != -1 else np.nan
        ...
        heart_axis_mapping = {axis: (idx if pd.notna(axis) else -1) for idx, axis in enumerate(df['heart_axis'].unique())}
        heart_axis = heart_axis_mapping[row['heart_axis']] if pd.notna(row['heart_axis']) and heart_axis_mapping[row['heart_axis']] != -1 else np.nan

        infarction_stadium1_mapping = {stadium: (idx if pd.notna(stadium) else -1) for idx, stadium in enumerate(df['infarction_stadium1'].unique())}
        infarction_stadium1 = infarction_stadium1_mapping[row['infarction_stadium1']] if pd.notna(row['infarction_stadium1']) and infarction_stadium1_mapping[row['infarction_stadium1']] != -1 else np.nan

        infarction_stadium2_mapping = {stadium: (idx if pd.notna(stadium) else -1) for idx, stadium in enumerate(df['infarction_stadium2'].unique())}
        infarction_stadium2 = infarction_stadium2_mapping[row['infarction_stadium2']] if pd.notna(row['infarction_stadium2']) and infarction_stadium2_mapping[row['infarction_stadium2']] != -1 else np.nan

        validated_by = row['validated_by']
        validated_by = int(validated_by) if is_integer(validated_by) else np.nan

        second_opinion = row['second_opinion']
        second_opinion = int(second_opinion) if pd.notna(second_opinion) else np.nan

        initial_autogenerated_report = row['initial_autogenerated_report']
        initial_autogenerated_report = int(initial_autogenerated_report) if pd.notna(initial_autogenerated_report) else np.nan

        validated_by_human = row['validated_by_human']
        validated_by_human = int(validated_by_human) if pd.notna(validated_by_human) else np.nan

        strat_fold = row['strat_fold']
        strat_fold = int(strat_fold) if is_integer(strat_fold) else np.nan
        # END ADDED
        ...
        # ADDED
        comment_lines += f'# Nurse: {nurse}\n# Site: {site}\n# Device: {device}\n# Year: {year}\n# Month: {month}\n# Day: {day}\n# Heart axis: {heart_axis}\n# Infarction stadium 1: {infarction_stadium1}\n# Infarction stadium 2: {infarction_stadium2}\n# Validated by: {validated_by}\n# Second opinion: {second_opinion}\n# Initial autogenerated report: {initial_autogenerated_report}\n# Validated by human: {validated_by_human}\n# Strat fold: {strat_fold}\n'
        # END ADDED
